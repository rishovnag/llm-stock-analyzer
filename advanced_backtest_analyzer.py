...
class AdvancedBacktestEngine:
    ...
    def fixed_split_backtest(self, df: pd.DataFrame, movement_column: str, 
                           ...
                           run_id: int) -> Optional[BacktestRun]:
        ...
        try:
            ...
            for i in range(lookback_window, len(test_data)):
                # Use combined train + test data up to current point for technical analysis
                ...
                # Calculate movement statistics from training data only
                movement_stats = self.movement_analyzer.calculate_movement_stats(
                    train_data, movement_column
                )
                
                # Generate prediction
                pred, conf = self.technical_rule_prediction(
    ...
    def rolling_window_backtest(self, df: pd.DataFrame, movement_column: str,
                              ...
                              step_size_months: int, run_id: int) -> List[BacktestRun]:
                ...
                for movement_type in movement_types:
                    ...
                    for train_ratio in train_ratios:
                        for run_num in range(num_runs_per_config):
                            ...
                            result = self.fixed_split_backtest(
                                df, movement_column, movement_type, adjusted_ratio, run_id
                            )
                    ...
                    for window_months, step_months in rolling_configs:
                        rolling_results = self.rolling_window_backtest(
                            df, movement_column, movement_type, window_months, step_months, run_id
                        )
...
if __name__ == "__main__":
    ...
    print("1. Quick test (fewer configurations)")
    print("2. Comprehensive analysis (full configuration)")
